...
def create_xml_for_vehicle(start, end):
                ...
                if cycle == 1:
                    ...
                    if new_df_eq.iloc[i]["Days_of_Week"] == 'Saturday' or new_df_eq.iloc[i]["Days_of_Week"] == 'Sunday':
                        ...
                        if Mon_roadway > 3:
                            skill='roadway'
                        Mon_roadway+=1  
                df_vehicle_72GM = df_vehicle_72GM.append(
                    pd.DataFrame(columns=df_vehicle_72GM.columns, data=[[new_df_eq.iloc[i]["vehicle_name"],
    ...
    new_df_vehicle_72GM = df_vehicle_72GM.copy()
    new_df_vehicle_72GM['start-time'] = new_df_vehicle_72GM['start-time'].apply(lambda x: fix_time(x))
    new_df_vehicle_72GM['end-time'] = new_df_vehicle_72GM['end-time'].apply(lambda x: fix_time(x))
    new_df_vehicle_72GM = new_df_vehicle_72GM[(new_df_vehicle_72GM['start-time'] > start) & (new_df_vehicle_72GM['end-time'] < end)]
    new_df_vehicle_72GM.to_csv('/home/usman/Downloads/Turf_Clusters/72GM/72GM_vehicles.csv')
