...
def create_xml_for_vehicle(start, end):
    ...
    #select the yard
    new_df_vehicle_580D = df_vehicle_580D[df_vehicle_580D['yard'] == 'OKY']
...
def create_xml_for_turf():
    df = pd.read_csv("/home/analytics/PycharmProjects/jsprit_create_xml/580D/OKY/services_yard_O'Keefe Yard")
        ...
        if int(row['start-time'] / 24) != int(row['end-time'] / 24):
            if row['Level'] == 'B1':
                ...
                if row['Id'].split("_")[1] == "0" or row['Id'].split("_")[2] == "0":
                    ...
                    start_at = fix_time("12d 06:00:00")
                    end_at = fix_time("14d 14:45:00")
                    xml.append('<start>{0}</start>'.format(start_at))
                    xml.append('<end>{0}</end>'.format(end_at))
                else:
...
if __name__ == '__main__':
    ...
    xml.append('</problem>')
    #print("\n".join(xml))
    with open('/home/analytics/PycharmProjects/jsprit_create_xml/580D/OKY/problem_setup.xml', 'w') as f:
        f.write("\n".join(xml))
