...
def create_xml_for_vehicle(start, end):
    ...
    #select the yard
    new_df_vehicle_580D = df_vehicle_580D[df_vehicle_580D['yard'] == 'GBY']
...
def create_xml_for_turf():
    df = pd.read_csv('/home/analytics/PycharmProjects/jsprit_create_xml/580D/GBY/services_yard_Gold Bar Yard')
...
def to_xml_turfs(df, filename=None, mode='w', start=0):
    ...
    def row_to_xml(row):
        ...
        xml.append('<duration>{0}</duration>'.format(row['service-duration']*0.80))
...
if __name__ == '__main__':
    ...
    xml.append('</problem>')
    #print("\n".join(xml))
    with open('/home/analytics/PycharmProjects/jsprit_create_xml/580D/GBY/problem_setup.xml', 'w') as f:
        f.write("\n".join(xml))
