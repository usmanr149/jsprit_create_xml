...
def create_xml_for_vehicle(start, end, required_skil):
    ...
    #select the yard
    new_df_vehicle_580D = df_vehicle_580D[df_vehicle_580D['yard'] == 'MWD']
    ...
    xml = ['<vehicles>']
    xml.append(new_df_vehicle_580D.to_xml_vehicle(skill=required_skil))
    xml.append('</vehicles>')
...
def create_xml_for_turf(required_skill):
    df = pd.read_csv('/home/analytics/PycharmProjects/jsprit_create_xml/580D/MWD/services_yard_MILLWOODS')
    ...
    xml = ['<services>']
    xml.append(df_580D.to_xml_turfs(required_skill=required_skill))
    xml.append('</services>')
...
def to_xml_turfs(df, filename=None, mode='w', required_skill='neighbourhood'):
    ...
    def row_to_xml(row):
        ...
        if row['required-skills'] == 'roadway':
            xml.append('<duration>{0}</duration>'.format(row['service-duration']*1.))
        else:
            xml.append('<duration>{0}</duration>'.format(row['service-duration'] * 1.))
        xml.append('<timeWindows>')
        xml.append('<timeWindow>')
        if row['required-skills'] == 'neighbourhood':
            if int(row['start-time'] / 24) != int(row['end-time'] / 24) and row['Level'] == 'B1':
...
if __name__ == '__main__':
    ...
    xml.append('</problem>')
    #print("\n".join(xml))
    with open('/home/analytics/PycharmProjects/jsprit_create_xml/580D/MWD/problem_setup.xml', 'w') as f:
        f.write("\n".join(xml))
