...
def create_xml_for_turf():
    df_72GM = pd.read_csv('/home/usman/Downloads/Turf_Clusters/ver1.3/72GM/ODL_Inputs_hold_test.csv')
...
def to_xml_turfs(df, filename=None, mode='w', start=0):
    ...
    def row_to_xml(row):
        ...
        xml.append('<locationId>{0}</locationId>'.format(str(row['Id']).split("_")[0].split('.')[0]))
...
if __name__ == '__main__':
    ...
    xml.append('</problem>')
    #print("\n".join(xml))
    with open('/home/usman/Downloads/Turf_Clusters/ver1.3/72GM/problem_setup.xml', 'w') as f:
        f.write("\n".join(xml))